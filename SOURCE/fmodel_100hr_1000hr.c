...
<rain infile> <humidity infile> <temperature infile> <previous day's moisture content> <output moisture content> <image size in total pixels> <lag>

rain file is rain duration in hours (unsigned 8bit)
humidity file is 1000% relative humidity
temperature file is LST in degrees K*10
previous day's moisture content in 100% (unsigned 8bit)
total pixels = rows*cols
lag = 100 or 1000

output is:
moisture uptake is the rate of moisture uptake by fuels	*10
moisture exchange is the rate of change in average moisture content *100
...
unsigned char *rain, *rain_ptr, *prev_lm, *prev_lm_ptr, *m_content, *m_content_ptr;
char filename[100];
...
lag=atoi(argv[7]);
 ...
 /** Check args, if incorrect specify usage **/
   ...
   {
      printf("Usage:\n\n");
      printf("<rain infile> <humidity infile> <temperature infile> <previous day's liter moisture>/  \n");
      printf(" <output moisture content> <image size in total pixels> <lag> \n\n");
...
Memory Allocation
*************************************************************/
allocate_binary(total_pixels, "uchar", (unsigned char *)(&rain));
...
if(read_binary(total_pixels, filename, "uchar", (unsigned char *)(&rain))==-1)
{
	printf("\n%s is missing!\n\n", filename);
	exit(0);
}
...
if(read_binary(total_pixels, filename, "sint", (signed short int *)(&humid))==-1)
 {
	printf("\n%s is missing!\n\n", filename);
	exit(0);
 }
...
if(read_binary(total_pixels, filename, "sint", (signed short int *)(&temp))==-1)
 {
	printf("\n%s is missing!\n\n", filename);
	exit(0);
 }
 ...
 if(read_binary(total_pixels, filename, "uchar", (unsigned char *)(&prev_lm)) ==-1)
 	{
...
for(i=0; i<total_pixels;i++)
    {
		...
		else if (lag == 100 || lag == 1000)
		{
			...
			*m_content_ptr=  (unsigned char)(*prev_lm_ptr + *m_exchange_ptr/100);
			...
			{
			printf("Invalid Lag Time\nPlease enter a lag of 100 or 1000\nexiting...");
			exit(0);
			}
...
float exchange(unsigned char up_rain,  unsigned char m_init, signed short int m_eq, int lag)
{
...
if (lag == 100)
	{
	m_b = ((delta_t-up_rain)*m_eq/10.0+up_rain*(0.5*up_rain+41))/delta_t;
	...
	{
	m_b = ((delta_t-up_rain)*m_eq/10.0+up_rain*(2.7*up_rain+76))/delta_t;
