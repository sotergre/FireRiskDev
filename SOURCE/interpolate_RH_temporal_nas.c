...
/**************************************************************************
 interpolate_MOD07_temporal_nas.c
 Jen Small
 ed Karyn Tabor
 last update 08/23/2013

 Distance-weighted interpolation for 32-bit float binary files.
 ...
 pixels is done, with 8 previous daily images and a distance weighted
 interpolation is performed to filled the pixel with a value.
 ...
 Compiles with:
 "gcc -g -o interpolate_MOD07_temporal_nas interpolate_MOD07_temporal_nas.c -lm"
...
***********************************************************************/
   ...
   {
      printf("Usage:\n\n");
      printf("interpolate_RH_temporal_nas maskfn in_dir out_dir year ");
   ...
   **********************************************************************/
   ...
   sprintf(infile_fn,"%sRH_MOD07_%d%.3d_Day_AMZEX.bin", in_dir, year,jday);
   read_binary(total_pixels,infile_fn,"float",(void *)(&infile));

   /*count file name*/
   sprintf(count_fn,"%sRH_MOD07_%d%.3d_Day_AMZEX_interp_count.bin", out_dir, year,jday);
   /*outfile name*/
   sprintf(outfile_fn,"%sRH_MOD07_%d%.3d_Day_AMZEX_interp.bin", out_dir, year,jday);
...
/**************************/
   /**********************************************************************
       Create 8-day average from previous week
      **********************************************************************/

   for(i=0; i<8; i++)
   {
   sprintf(infile_fn,"%sRH_MOD07_%d%.3d_Day_AMZEX.bin", in_dir, date_array[i][0],date_array[i][1]);
