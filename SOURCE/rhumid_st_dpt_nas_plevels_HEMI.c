/******************************************************************************************
rhumid_st_dpt_nas_plevels.c

Calculates relative humidity from dew point temperature and temperature. Has to read the surface pressure file first to know which pressure level for the DPT.

last update =6/12/2013

Input: 32 bit float dewpoint, temperature, and surface pressure files

Output: binary signed 32 bit float relative humidity 1000%
...
sprintf(SP_filename, "%sSF_Pressure/%d/SP_MOD07_%d%.3d_Day_HEMI.bin", outdir, year, year, jday);
...
sprintf(LST_filename, "%sSFT/%d/SFT_MOD07_%d%.3d_Day_HEMI.bin", outdir, year, year, jday);
...
for (i=min; i<17; i++)
{
	sprintf(DPT_filename, "%sTP_PROFILE/%d/DPT_%d_MOD07_%d%.3d_Day_HEMI.bin", outdir, year, plevel[i][0],year, jday);
...
*************************************************************************/
sprintf(RH_filename,  "%sRH/%d/RH_MOD07_%d%.3d_Day_HEMI.bin", outdir, year,year, jday);
