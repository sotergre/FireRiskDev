...
/***********************************************************************
 ...
 C function, reads a binary image.


 prototype:

   int read_binary(int numpx, char *fname, char *dtype, void **imptr)
 ...
 return value:
   ...
   For a successful read, returns the number of bytes read.
...
int read_binary(int numpx, char *fname, char *dtype, void **imptr)
{
   ...
   if( im_fd==-1 )
   {
      ...
      printf("Make sure the file exists and you have read permission.\n");
      printf("Exiting...\n");
      return -1;
   }
