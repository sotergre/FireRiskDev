...
/**************************************************************************
 interpolate_SFT_temporal_nas_16day.c
 Karyn Tabor
...
***********************************************************************/
   ...
   **********************************************************************/
   ...
   sprintf(infile_fn,"%sSFT_MOD07_%d%.3d_Day_AMZEX.bin", in_dir, year,jday);
   read_binary(total_pixels,infile_fn,"float",(void *)(&infile));

   /*count file name*/
   sprintf(count_fn,"%sSFT_MOD07_%d%.3d_Day_AMZEX_interp_count.bin", out_dir, year,jday);
   /*outfile name*/
   sprintf(outfile_fn,"%sSFT_MOD07_%d%.3d_Day_AMZEX_interp.bin", out_dir, year,jday);
...
/**************************/
   /**********************************************************************
       Create 16-day average from previous week
      **********************************************************************/

   for(i=0; i<16; i++)
   {
   sprintf(infile_fn,"%sSFT_MOD07_%d%.3d_Day_AMZEX.bin", in_dir, date_array[i][0],date_array[i][1]);
