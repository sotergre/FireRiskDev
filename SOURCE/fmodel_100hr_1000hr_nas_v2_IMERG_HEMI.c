...
sprintf(filename, "%s/HEMI/IMERG/%d/daysum/ppt_IMERG30m_early_%d%.3d_dur_HEMI.bin" , data_dir, prevyear, prevyear,prevjday);
...
sprintf(filename, "%s/HEMI/MODOUT/RH/%d/RH_MOD07_%d%.3d_Day_HEMI_interp.bin" , data_dir, year, year,jday);
...
sprintf(filename, "%s/HEMI/MODOUT/SFT/%d/SFT_MOD07_%d%.3d_Day_HEMI_interp.bin" , data_dir, year, year,jday);
...
sprintf(filename, "%s/HEMI/MODOUT/LM/%d/HR%.4d/LM%.4dh_%d%.3d_Day_HEMI_last.bin" , data_dir, prevyear, lag, lag, prevyear,prevjday);
...
sprintf(filename, "%s/HEMI/MODOUT/LM/%d/HR%.4d/LM%.4dh_%d%.3d_Day_HEMI_count.bin" , data_dir, prevyear, lag,lag, prevyear,prevjday);
...
sprintf(filename, "%s/HEMI/MODOUT/LM/%d/HR%.4d/LM%.4dh_%d%.3d_Day_HEMI.bin" , data_dir, year, lag, lag, year,jday);
write_binary(total_pixels, filename, "sint", (signed short int *)(&m_content));

sprintf(filename, "%s/HEMI/MODOUT/LM/%d/HR%.4d/LM%.4dh_%d%.3d_Day_HEMI_last.bin" , data_dir, year, lag, lag, year,jday);
write_binary(total_pixels, filename, "sint", (signed short int *)(&prev_lm));

sprintf(filename, "%s/HEMI/MODOUT/LM/%d/HR%.4d/LM%.4dh_%d%.3d_Day_HEMI_count.bin", data_dir, year, lag, lag, year,jday);
