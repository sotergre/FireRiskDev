...
unsigned char *rain, *rain_ptr;
char filename[100];
char content_filename[101];
char data_dir[101];

float equilibrium(signed short int, signed short int);
float exchange(unsigned char, signed short int, signed short int, int);
...
sprintf(filename, "%s/AMZEX/3B42RT/%d/daysum/ppt_3b42rt_%d%.3d_dur_AMZEX.bin" , data_dir, prevyear, prevyear,prevjday);
if(read_binary_fmodel(total_pixels, filename, "uchar", (unsigned char *)(&rain))==-1)
{
	printf("\n%s is missing!\n\n", filename);
	exit(0);
}
...
sprintf(filename, "%s/AMZEX/MODOUT/LM/%d/HR%.4d/LM%.4dh_%d%.3d_Day_AMZEX_last.bin" , data_dir, prevyear, lag, lag, prevyear,prevjday);
 ...
 sprintf(filename, "%s/AMZEX/MODOUT/LM/%d/HR%.4d/LM%.4dh_%d%.3d_Day_AMZEX_count.bin" , data_dir, prevyear, lag,lag, prevyear,prevjday);
...
sprintf(filename, "%s/AMZEX/MODOUT/LM/%d/HR%.4d/LM%.4dh_%d%.3d_Day_AMZEX.bin" , data_dir, year, lag, lag, year,jday);
write_binary(total_pixels, filename, "sint", (signed short int *)(&m_content));

sprintf(filename, "%s/AMZEX/MODOUT/LM/%d/HR%.4d/LM%.4dh_%d%.3d_Day_AMZEX_last.bin" , data_dir, year, lag, lag, year,jday);
write_binary(total_pixels, filename, "sint", (signed short int *)(&prev_lm));

sprintf(filename, "%s/AMZEX/MODOUT/LM/%d/HR%.4d/LM%.4dh_%d%.3d_Day_AMZEX_count.bin", data_dir, year, lag, lag, year,jday);
...
float exchange(unsigned char up_rain,  signed short int m_init, signed short int m_eq, int lag)
{
