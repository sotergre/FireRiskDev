...
sprintf(SP_filename, "%sSF_Pressure/%d/SP_MOD07_%d%.3d_Day_IDN.bin", outdir, year, year, jday);
...
sprintf(LST_filename, "%sSFT/%d/SFT_MOD07_%d%.3d_Day_IDN.bin", outdir, year, year, jday);
...
for (i=min; i<17; i++)
{
	sprintf(DPT_filename, "%sTP_PROFILE/%d/DPT_%d_MOD07_%d%.3d_Day_IDN.bin", outdir, year, plevel[i][0],year, jday);
...
*************************************************************************/
sprintf(RH_filename,  "%sRH/%d/RH_MOD07_%d%.3d_Day_IDN.bin", outdir, year,year, jday);
