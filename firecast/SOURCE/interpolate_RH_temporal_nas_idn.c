...
***********************************************************************/
   ...
   **********************************************************************/
   ...
   sprintf(infile_fn,"%sRH_MOD07_%d%.3d_Day_IDN.bin", in_dir, year,jday);
   read_binary(total_pixels,infile_fn,"float",(void *)(&infile));

   /*count file name*/
   sprintf(count_fn,"%sRH_MOD07_%d%.3d_Day_IDN_interp_count.bin", out_dir, year,jday);
   /*outfile name*/
   sprintf(outfile_fn,"%sRH_MOD07_%d%.3d_Day_IDN_interp.bin", out_dir, year,jday);
...
/**************************/
   ...
   for(i=0; i<8; i++)
   {
   sprintf(infile_fn,"%sRH_MOD07_%d%.3d_Day_IDN.bin", in_dir, date_array[i][0],date_array[i][1]);
