...
export set mask="${DATA_PATH}/IDN/Mask/IDN_Mask1.bin"
if [ ! -e "$mask" ]
then
...
echo "Enter Process"
	...
	then
		...
		echo "" >> ${prefix}07${year}$(printf "%.3d" $jday)MOD07L2SP.batch
		echo "set_projection_to_geometry   ProjectionTo=\"Sinusoidal\" WidthTo=1031 HeightTo=406 PixelSizeXTo=$pixel_size_m PixelSizeYTo=$pixel_size_m LatitudeMinTo=$LR_Y LatitudeMaxTo=$UL_Y LongitudeMinTo=$UL_X LongitudeMaxTo=$LR_X CenterLongitudeTo=119.0"  >> ${prefix}07${year}$(printf "%.3d" $jday)MOD07L2SP.batch
		...
		for profile in 13 14 15 16 17 18 19 20
			...
			echo "" >> ${prefix}07${year}$(printf "%.3d" $jday)MOD07L2DPT.batch
			echo "set_projection_to_geometry   ProjectionTo=\"Sinusoidal\" WidthTo=1031 HeightTo=406 PixelSizeXTo=$pixel_size_m PixelSizeYTo=$pixel_size_m LatitudeMinTo=$LR_Y LatitudeMaxTo=$UL_Y LongitudeMinTo=$UL_X LongitudeMaxTo=$LR_X CenterLongitudeTo=119.0 "  >> ${prefix}07${year}$(printf "%.3d" $jday)MOD07L2DPT.batch
		...
		echo "" >> ${prefix}07${year}$(printf "%.3d" $jday)MOD07L2AT.batch
		echo "set_projection_to_geometry   ProjectionTo=\"Sinusoidal\"  CenterLongitudeTo=0.0 WidthTo=1031 HeightTo=406 PixelSizeXTo=$pixel_size_m PixelSizeYTo=$pixel_size_m LatitudeMinTo=$LR_Y LatitudeMaxTo=$UL_Y LongitudeMinTo=$UL_X LongitudeMaxTo=$LR_X " >> ${prefix}07${year}$(printf "%.3d" $jday)MOD07L2AT.batch
		...
		echo "" >> ${prefix}07${year}$(printf "%.3d" $jday)MOD07L2SFT.batch	
		echo 'set_projection_to_geometry   ProjectionTo='Sinusoidal'  CenterLongitudeTo=0.0 WidthTo=1031 HeightTo=406 PixelSizeXTo='$pixel_size_m' PixelSizeYTo='$pixel_size_m' LatitudeMinTo='$LR_Y' LatitudeMaxTo='$UL_Y' LongitudeMinTo='$UL_X' LongitudeMaxTo='$LR_X' '  >> ${prefix}07${year}$(printf "%.3d" $jday)MOD07L2SFT.batch	
	...
	######	interpolate	#####
		...
		then
			...
			${home_dir}/interpolate_RH_temporal_nas_32day ${mask} ${outputdirectory}RH/${year}/ ${outputdirectory}RH/${year}/ ${year} ${jday} ${ncols} ${nrows} -1
			...
			echo "ncols 1031" > RH_${prefix}07_${year}$(printf "%.3d" $jday)_${pass}_IDN_interp_count.hdr
			echo "nrows 406" >> RH_${prefix}07_${year}$(printf "%.3d" $jday)_${pass}_IDN_interp_count.hdr
			...
			echo "XULCENTER 10521164.9676" >> RH_${prefix}07_${year}$(printf "%.3d" $jday)_${pass}_IDN_interp_count.hdr
			echo "YULCENTER 812797.1942" >> RH_${prefix}07_${year}$(printf "%.3d" $jday)_${pass}_IDN_interp_count.hdr
		...
		then
			...
			${home_dir}/interpolate_SFT_temporal_nas_32day ${mask} ${outputdirectory}SFT/${year}/ ${outputdirectory}SFT/${year}/ ${year} ${jday} ${ncols} ${nrows} -1
			cd ${outputdirectory}SFT/${year}/
			...
			echo "ULXMAP 10521164.9676" >> SFT_${prefix}07_${year}$(printf "%.3d" $jday)_${pass}_IDN_interp_count.hdr
			echo "ULYMAP 812797.1942" >> SFT_${prefix}07_${year}$(printf "%.3d" $jday)_${pass}_IDN_interp_count.hdr
			echo "nodata_value 0" >> SFT_${prefix}07_${year}$(printf "%.3d" $jday)_${pass}_IDN_interp_count.hdr
...
# 
		...
		##############################################
            ...
            cd /firecast/DATA/IDN/MODOUT/DSLR/2015/
            ...
            gdal_translate -ot Byte -of GTiff -stats -strict DSLR_${year}$(printf "%.3d" $jday)_IDN.bin DSLR_${year}$(printf "%.3d" $(( $jday )))_IDN_Temp.tif
            gdalwarp -t_srs EPSG:3857 -ot Byte -of GTiff -overwrite DSLR_${year}$(printf "%.3d" $jday)_IDN_Temp.tif DSLR_${year}$(printf "%.3d" $jday)_IDN.tif
